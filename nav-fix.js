...
function showPage(name,push){
  ...
  if(push!==false){
    _histLock=true;
    try{history.pushState({p:name},"","#/"+name);}catch(e){}
    _histLock=false;
  }
...
function workerList(){
  var list=db.workers||[];
  if(!list.length){
    list=Object.values(db.members||{}).filter(function(m){return m.role&&m.role!=="user";}).map(function(m,i){
      return {name:m.name,phone:m.phone,pad:m.role,work:"समिति सेवा",level:i+1};
    });
  }
...
function sevaOf(w){
  ...
  return (db.donations||[]).filter(function(d){return (p&&d.uid===p)||(n&&d.name===n);}).reduce(function(s,d){return s+Number(d.amt||0);},0);
}
let _wi=0,_wt=null;
function showWorker(){
  ...
  var w=list[_wi++];
  var amt=sevaOf(w);
  el.innerHTML='<div class="wcard"><div class="wrank">#'+(w.level||"–")+'</div><div class="wbody"><b>'+(w.name||"")+'</b><div class="meta">'+(w.pad||"कार्यकर्ता")+(w.work?(" • "+w.work):"")+'</div><div class="wseva">इनके तरफ़ से सेवा प्राप्त: <b>'+inr(amt)+'</b></div></div></div>';
}
const _rh2=window.renderHome;
window.renderHome=function(){
  if(typeof _rh2==="function")try{_rh2();}catch(e){}
  showWorker();
  if(_wt)clearInterval(_wt);
  _wt=setInterval(showWorker,3800);
};
